...
class Trainer(object):
    ...
    def _train_step(self, batch):
        ...
        if self.steps > self.config.train.discriminator_train_start_steps:
            # re-compute y_
            with torch.no_grad():
                y_ = self.model["generator"](mel, spec_lengths, f0)[2]
    ...
    def _eval_step(self, batch):
        ...
        # generator forward
        # outs: [mu, logvar, y_, s]
        outs = self.model["generator"](mel, spec_lengths, f0)
        y_ = outs[2]
        # calculate kl divergence loss
        mu, logvar = outs[0], outs[1]
        kl_loss = self.criterion["kl"](mu, logvar)
        gen_loss = self.config.train.lambda_kl * kl_loss
    ...
    def _genearete_and_save_intermediate_result(self, batch):
        ...
        # generator forward
        # outs: mu, log_var, x, s
        outs = self.model["generator"](mel, spec_lengths, f0)[2:]
...
def main(config: DictConfig) -> None:
    ...
    collater = Collater(
        ...
        upsample_scales=config.generator.decoder_params.upsample_scales,
    )
