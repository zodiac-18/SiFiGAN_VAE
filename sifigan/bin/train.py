...
class Trainer(object):
    ...
    def _train_step(self, batch):
        ...
        for name, out in outs.items():
            ...
            if name == "out_trans_2":
                latent_mse_loss = self.criterion["latent"](z_trans_1, z_trans_2)
                gen_loss += self.config.train.lambda_latent * latent_mse_loss
                self.total_train_loss["train/latent_loss"] += latent_mse_loss.item()
    ...
    def _eval_step(self, batch):
        ...
        for name, out in outs.items():
            ...
            if name == "out_trans_2":
                latent_mse_loss = self.criterion["latent"](z_trans_1, z_trans_2)
                gen_loss += self.config.train.lambda_latent * latent_mse_loss
                self.total_eval_loss["eval/latent_loss"] += latent_mse_loss.item()
...
class Collater(object):
    ...
    def __call__(self, batch):
        ...
        for idx in range(len(batch)):
            x, c, f0 = batch[idx]
            if c.shape[1] > self.batch_max_frames:
                # randomly pickup with the batch_max_length length of the part
                start_frame = np.random.randint(0, c.shape[1] - self.batch_max_frames)
                ...
                spec_lengths[idx] = c.shape[1]
                for i in range(len(c)):
                    self._check_length(y, c[i], f0)
            else:
        ...
        c_batch = torch.FloatTensor(np.array(c_batch)).transpose(2, 3).transpose(2, 1)  # (B, T, f0_aug, D) -> (B, D, f0_aug. T)
    ...
    def _check_length(self, x, c, f0):
        ...
        #logger.info(f"x: {len(x)}, c: {len(c)}, f0: {len(f0)}")
...
def main(config: DictConfig) -> None:
    ...
    criterion = {
        ...
        "latent": hydra.utils.instantiate(config.train.latent_loss).to(device),
    }
